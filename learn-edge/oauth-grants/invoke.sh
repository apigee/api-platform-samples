...
printf  "\n\nFetching consumer key and secret for Learn Edge App from Apigee Edge..."
...
accesstoken_response=`curl -v -X POST -H  "Authorization: Basic $auth" "https://$org-$env.$api_domain/v1/learn-edge/token?grant_type=client_credentials" 2>/dev/null`

printf  "curl -v -X POST -H \"Authorization: Basic $auth\" \"https://$org-$env.$api_domain/v1/learn-edge/token?grant_type=client_credentials\" \n"
