#!/bin/sh
echo Deleting any existing /responses directory
rm -r ./responses
...
echo This script will run you through complete set up of an API facade and supporting objects like developers, apps, and API products, using the Apigee API
...
curl -u {$username}:{$password} -H "Content-type: application/json" -X POST -d'{"connection" : {"basePath" : "/weather","virtualHost" : [ "default" ]}, "name" : "default", "routeRule" : [ {"name" : "default","targetEndpoint" : "default"} ]}' https://api.enterprise.apigee.com/v1/o/$org/apis/weatherAPI/revisions/1/proxies > ./responses/CreateProxyEndpoint.json
...
echo Detach Quota Policy


curl -u {$username}:{$password} -H "Content-Type: application/octet-stream" -X DELETE https://api.enterprse.apigee.com/v1/o/{$myorg}/apis/weatherAPI/revisions/1/proxies/default/flows/PreFlow/policies/WeatherQuota?'enforcement=request' -v > ./responses/detachPolicy.json
