...
def getDeployments():
    ...
    resp = httpCall('GET',
            '/v1/organizations/%s/sharedflows/%s/deployments' \
                % (Organization, Name),
            hdrs, None)
...
if Directory != None:
    ...
    for dirPath, dirNames, fileNames in dirList:
        # If we include more than the sharedflowbundle folder, 
        # the import will fail.
        if 'sharedflowbundle' in dirPath:
            if not pathContainsDot(dirPath):
                for fileEntry in fileNames:
                    if not fileEntry.endswith('~'):
                        fn = os.path.join(dirPath, fileEntry)
                        en = os.path.join(
...
uri = '/v1/organizations/%s/sharedflows?action=import&name=%s' % \
            (Organization, Name)
...
print 'Imported new shared flow version %i' % revision
...
if ShouldDeploy:
    ...
    for d in deps:
        if d['environment'] == Environment and \
            ...
            print 'Undeploying /v1/organizations/%s/environments/%s/sharedflows/%s/revisions/%i/deployments' % \
            (Organization, Environment, Name, d['revision'])
            conn = httplib.HTTPSConnection(httpHost)
            resp = httpCall('DELETE', 
                ('/v1/organizations/%s/environments/%s/sharedflows/%s/revisions/%i/deployments') % \
                        (Organization, Environment, Name, d['revision']),
                 None, None)
    ...
    # Deploy the bundle
    print 'Deploying /v1/organizations/%s/environments/%s/sharedflows/%s/revisions/%i/deployments' % \
    (Organization, Environment, Name, revision)
